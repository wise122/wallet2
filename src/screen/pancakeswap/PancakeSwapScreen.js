...
export default function PancakeSwapScreen({navigation}){
    ...
    const reset = async () => {
        ...
        token.balance = await PancakeswapModule.tokenBalance(rawActiveWallet, fromToken);
        setFromToken(token);
        setFromValue('');
        //reset toToken
        const token2 = {...toToken};
        token2.balance = await PancakeswapModule.tokenBalance(rawActiveWallet, token2)
        setToToken(token2);
        setToValue('');
        //reset Trade
        setTrade([]);
    ...
    const exchange = async (from,to,value) => {
        if (isValidTrade(value)) {
            setError('Loading...')
            const finalValue = value;
            const exchange = await PancakeswapModule.trade(activeWallet.address, from, to, finalValue);
        ...
        <SafeAreaView style={styles.container}>
            <View style={styles.contentContainer}>
                            ...
                            <LMTouchableOpacity style={styles.coin} onPress={()=>{
                                navigation.navigate("TokenScreen",{
                                    onSelect : async (token) => {
                                        const balance = await PancakeswapModule.tokenBalance(rawActiveWallet, token);
                ...
                <View style={styles.exchangeRate}>
                    <Text>PancakeSwap {trade.uniswapVersion}</Text>
                ...
                <View style={{marginTop: 0}}>
                    <LMButton
                        label={error}
                        style={{backgroundColor : error == 'Swap' ? '#f5bd00' : red}}
                        onPress={async () => {
                        if(error == 'Swap'){
                            await swap();
                        }
